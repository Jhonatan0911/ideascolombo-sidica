...
if(!empty($_SERVER['HTTP_X_REQUESTED_WITH']) && strtolower($_SERVER['HTTP_X_REQUESTED_WITH']) == 'xmlhttprequest') 
{
    ...
    //imagen 1
	$file = $_FILES['img_porta_add']['name'];
	$type = $_FILES['img_porta_add']['type'];
	$tmp = $_FILES['img_porta_add']['tmp_name'];
	

	//obtenemos la informacion del portafolio
	$id_tipoPorta = $_POST['tipo_porta'];
	...
	if(isset($file) && ($type === 'image/png'||$type === 'image/jpeg'||$type === 'image/jpg')){
		...
		if ($file && move_uploaded_file($_FILES['img_porta_add']['tmp_name'],"../../img/portfolio/".$file))
		{
			
			$portafolio_new = "INSERT INTO portafolio(nameImg, tipo) VALUES ('$file',$id_tipoPorta)";
			$result = $bd->consulta($portafolio_new);

			if($result){
				echo "Portafolio nuevo almacenado";
			}else{
